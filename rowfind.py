...
def find_rows(coords: tuple[tuple[int, int]] | list[tuple[int, int]], row_size: int) -> tuple[tuple[int, int]]:
    """
    ...
    for x, y in coords:
        for dx, dy in walk:
            if (x + (row_size - 1) * dx, y + (row_size - 1) * dy) not in coords_set:
                continue
...
def main() -> None:
    ...
    try:
        ...
        print(f"error: invalid row size: {sys.argv[1]} (should be an integer greater than 1)")
        sys.exit(1)
